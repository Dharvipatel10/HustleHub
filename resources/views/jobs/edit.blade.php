        ...
        <h2 class="text-4xl text-center font-bold mb-4">
            Edit Job Listing
                ...
                <form method="POST" action="{{route('jobs.update', $job->id)}}" enctype="multipart/form-data">
                    ...
                    <x-inputs.text id="title" name="title" label="Job Title" placeholder="Software Developer" :value="old('title', $job->title)"/>
                    ...
                    <x-inputs.select id="job_type" name="job_type" label="Job Type" :value="old('job_type', $job->job_type)" :options="['full_time' =>'full_time', 'part_time' => 'part_time', 'contract' => 'contract', 'temporary' => 'temporary', 'Volunteer' => 'Volunteer', 'On-Call' => 'On-Call']"/>
                    
                    <x-inputs.select id="remote" name="remote" label="Remote" :options="[0 => 'No', 1 => 'Yes']" :value="old('remote', $job->remote)" />

                    <x-inputs.text id="address" name="address" label="Address" placeholder="Wittenberger str. 5" :value="old('address', $job->address)"/>

                    <x-inputs.text id="city" name="city" label="City" placeholder="Chemnitz" :value="old('city', $job->city)"/>

                    <x-inputs.text id="state" name="state" label="State" placeholder="Saxony" :value="old('state', $job->state)"/>

                    <x-inputs.text id="zipcode" name="zipcode" label="ZIP Code" placeholder="09126" :value="old('zip_code', $job->zip_code)"/>
                    ...
                    <x-inputs.text id="company_website" name="company_website" label="Company Website" type="url" placeholder="Enter Company Website" :value="old('company_website', $job->company_website)"/>

                    <x-inputs.text id="contact_phone" name="contact_phone" label="Contact Phone" placeholder="Enter Contat Phone" :value="old('contact_phone', $job->contact_phone)"/>
